...
def _handle_seller_incoming(incoming_msg, resp: MessagingResponse):
    msg_lower = incoming_msg.strip()
    # Seller asking for help from supervisor
    if any(k in msg_lower.lower() for k in SELLER_HELP_KEYWORDS):
        # find active buyer(s) in queue; we will notify supervisor and mark session(s)
        buyers = list_handoff_buyers_for_seller(SELLER_NUMBER)
        if not buyers:
            resp.message("No active buyer conversation found to escalate.")
            return str(resp)
        # notify supervisor (you) and mark each as handoff_supervisor
        for b in buyers:
            sess = get_session(b)
            ...
            if SUPERVISOR_NUMBER:
                client.messages.create(
                    ...
                    body=f"⚠️ Seller requested supervisor for chat with {b}.\nSeller note: {incoming_msg}\nReply here to join the conversation."
                )
        resp.message("Supervisor notified. They will join shortly.")
        return str(resp)

    # Seller-level commands
    lower = msg_lower.lower().strip()
    if lower == "#help":
        resp.message("Seller commands: #list, #switch <whatsapp:+...>, #bot, #end, #supervisor")
        return str(resp)
    ...
    if lower.startswith("#switch"):
        ...
        if len(parts) == 2:
            target = parts[1]
            # verify session exists and in handoff
            s = get_session(target)
            if s.get("state") not in ("handoff_seller", "handoff_supervisor"):
                resp.message(f"{target} is not in an active handoff.")
                return str(resp)
            # set the target as the active one by linking seller => this is simple for single-seller MVP
            s["linked_seller"] = SELLER_NUMBER
            set_session(target, s)
            resp.message(f"Switched active buyer to {target}.")
            return str(resp)
        else:
            resp.message("Usage: #switch whatsapp:+2547XXXXXXXX")
            return str(resp)

    if lower == "#bot":
        # return buyer(s) to bot flow (for simplicity operate on all linked buyers)
        buyers = list_handoff_buyers_for_seller(SELLER_NUMBER)
        if not buyers:
            resp.message("No active buyers to return to bot.")
            return str(resp)
        for b in buyers:
            s = get_session(b)
            ...
            set_session(b, s)
            client.messages.create(from_=TWILIO_WHATSAPP_NUMBER, to=b, body="🔄 You're back with the bot. Type 'menu' to see products.")
        resp.message("Returned active buyer(s) to bot flow.")
        return str(resp)

    if lower == "#end":
        buyers = list_handoff_buyers_for_seller(SELLER_NUMBER)
        if not buyers:
            resp.message("No active buyers to end.")
            return str(resp)
        for b in buyers:
            client.messages.create(from_=TWILIO_WHATSAPP_NUMBER, to=b, body="✅ Chat closed by seller. Thank you!")
            pop_session(b)
        resp.message("Closed chats with active buyers.")
        return str(resp)
